...
-- Library functions 
    -- Misc functions
        ...
        function Library:ConvertEnum(enum)
            ...
            for i = 2, #EnumParts do
                ...
                if not EnumItem then
                    error("Invalid: " .. table.concat(EnumParts, "."))
                end
                EnumTable = EnumItem
            end
        ...
        function Library:UpdateConfigList()
            ...
            for _, file in listfiles(Library.Directory .. "/configs") do
                local Name = file:match("([^/\\]+)%.cfg$") or file:gsub("%.cfg$", "")
                List[#List + 1] = Name
            end
        ...
        function Library:GetConfig()
            ...
            for Idx, Value in Flags do
                if type(Value) == "table" and (Value.key ~= nil or Value.Key ~= nil) then
                    local key = Value.Key or Value.key
                    local active = Value.Active or Value.active
                    local mode = Value.Mode or Value.mode

                    Config[Idx] = { Active = active, Mode = mode, Key = tostring(key) }
            ...
            function Library:LoadConfig(JSON)
                ...
                for Idx, Value in Config do
                    ...
                    if Function then
                        ...
                        elseif type(Value) == "table" and (Value.Active ~= nil or Value.active ~= nil) then
                            local tbl = {
                                Mode = Value.Mode or Value.mode,
                                Active = (Value.Active ~= nil) and Value.Active or Value.active,
                                Key = Value.Key or Value.key
                            }
                            Function(tbl)
                        else
                            Function(Value)
    ...
    -- Library element functions
        function Library:Window(properties)
            ...
            local Items = Cfg.Items; do
                ...
                -- Watermark
                    ...
                    Library:Create( "ImageLabel" , {
                        BorderColor3 = rgb(0, 0, 0);
                        Parent = Items.Watermark;
                        Image = "rbxassetid://135955819712566";
                        BackgroundTransparency = 1;
            ...
            button.Size = UDim2.new(0, 0.1, 0, 0.1)
            ...
            circle.Visible = true
            circle.Color = themes.preset.accent
            circle.Thickness = 1
            circle.Radius = 3
            circle.Filled = true

local function update_mouseicon()
    local mousePos = InputService:GetMouseLocation()
    circle.Position = mousePos
end

RunService.RenderStepped:Connect(update_mouseicon)
            ...
            function Cfg.ToggleMenu(bool) 
                ...
                local TweenEverythingNigga = Library:Fade(Items.Window, "BackgroundTransparency", bool)
                Library:Connection(TweenEverythingNigga.Completed, function()
                    Cfg.Tweening = false
                    if not bool then
        ...
        function Library:Configs(window) 
            ...
            Section:Button({Name = "Save", Callback = function() 
                ...
                Notifications:Create({Name = "Saved Config (" .. ConfigText .. ".cfg" .. ")"}) 
            end})

            Section:Button({Name = "Load", Callback = function() 
                Library:LoadConfig(readfile(Library.Directory .. "/configs/" .. ConfigText .. ".cfg"))  
                Library:UpdateConfigList() 
                Notifications:Create({Name = "Loaded Config ("  .. ConfigText .. ".cfg" .. ")"}) 
            end})

            Section:Button({Name = "Delete", Callback = function() 
                delfile(Library.Directory .. "/configs/" .. ConfigText .. ".cfg")  
                Library:UpdateConfigList() 
                Notifications:Create({Name = "Deleted Config (" .. ConfigText .. ".cfg" .. ")"}) 
            end})
            ...
            end, Default = true, Key = Enum.KeyCode.Insert})
